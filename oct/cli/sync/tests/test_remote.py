...
class SyncRemoteTestCase(PlaybookRunnerTestCase):
    def test_no_repo(self):
        self.run_test(
            TestCaseParameters(
                args=['sync', 'remote'], expected_result=CLICK_RC_USAGE, expected_output='Missing argument "repository"'
            )
        )
    ...
    def test_repository(self):
        self.run_test(
            TestCaseParameters(
                args=['sync', 'remote', Repository.origin], expected_calls=[{
    ...
    def test_bad_repository(self):
        self.run_test(
            TestCaseParameters(
                args=['sync', 'remote', Repository.enterprise], expected_result=CLICK_RC_USAGE,
    ...
    def test_remote(self):
        self.run_test(
            TestCaseParameters(
                args=['sync', 'remote', Repository.origin, '--remote', 'myupstream'], expected_calls=[{
                    'playbook_relative_path': 'sync/remote',
                    'playbook_variables': {
                        'origin_ci_sync_repository': Repository.origin,
    ...
    def test_remotes(self):
        self.run_test(
            TestCaseParameters(
                ...
                expected_result=CLICK_RC_USAGE, expected_output='new remote and existing remote cannot be specified at once'
            )
        )
    ...
    def test_explicit_destination(self):
        self.run_test(
            TestCaseParameters(
                args=['sync', 'remote', Repository.origin, '--dest', '/some/remote/path'], expected_calls=[{
                    'playbook_relative_path': 'sync/remote',
                    'playbook_variables': {
                        'origin_ci_sync_repository': Repository.origin,
                        'origin_ci_sync_remote': 'origin',
    ...
    def test_commit(self):
        self.run_test(
            TestCaseParameters(
                args=['sync', 'remote', Repository.origin, '--commit', 'SHA'], expected_calls=[{
                    'playbook_relative_path': 'sync/remote',
                    'playbook_variables': {
                        'origin_ci_sync_repository': Repository.origin,
                        'origin_ci_sync_remote': 'origin',
    ...
    def test_branch(self):
        self.run_test(
            TestCaseParameters(
                args=['sync', 'remote', Repository.origin, '--branch', 'myfeature'], expected_calls=[{
                    'playbook_relative_path': 'sync/remote',
                    'playbook_variables': {
                        'origin_ci_sync_repository': Repository.origin,
                        'origin_ci_sync_remote': 'origin',
    ...
    def test_tag(self):
        self.run_test(
            TestCaseParameters(
                args=['sync', 'remote', Repository.origin, '--tag', 'v1.0.0'], expected_calls=[{
                    'playbook_relative_path': 'sync/remote',
                    'playbook_variables': {
                        'origin_ci_sync_repository': Repository.origin,
                        'origin_ci_sync_remote': 'origin',
