...
class SyncLocalTestCase(PlaybookRunnerTestCase):
    def test_no_repo(self):
        self.run_test(
            TestCaseParameters(
                args=['sync', 'local'], expected_result=CLICK_RC_USAGE, expected_output='Missing argument "repository"'
            )
        )
    ...
    def test_repository(self):
        self.run_test(
            TestCaseParameters(
                args=['sync', 'local', Repository.origin], expected_calls=[{
    ...
    def test_explicit_source(self):
        ...
        self.run_test(
            TestCaseParameters(
                args=['sync', 'local', Repository.origin, '--src', abspath(dirname(__file__))], expected_calls=[{
                    'playbook_relative_path': 'sync/local',
                    'playbook_variables': {
                        'origin_ci_sync_repository': Repository.origin,
    ...
    def test_explicit_destination(self):
        self.run_test(
            TestCaseParameters(
                args=['sync', 'local', Repository.origin, '--dest', '/some/remote/path'], expected_calls=[{
                    'playbook_relative_path': 'sync/local',
                    'playbook_variables': {
                        'origin_ci_sync_repository': Repository.origin,
    ...
    def test_commit(self):
        self.run_test(
            TestCaseParameters(
                args=['sync', 'local', Repository.origin, '--commit', 'SHA'], expected_calls=[{
                    'playbook_relative_path': 'sync/local',
                    'playbook_variables': {
                        'origin_ci_sync_repository': Repository.origin,
    ...
    def test_branch(self):
        self.run_test(
            TestCaseParameters(
                args=['sync', 'local', Repository.origin, '--branch', 'myfeature'], expected_calls=[{
                    'playbook_relative_path': 'sync/local',
                    'playbook_variables': {
                        'origin_ci_sync_repository': Repository.origin,
    ...
    def test_tag(self):
        self.run_test(
            TestCaseParameters(
                args=['sync', 'local', Repository.origin, '--tag', 'v1.0.0'], expected_calls=[{
                    'playbook_relative_path': 'sync/local',
                    'playbook_variables': {
                        'origin_ci_sync_repository': Repository.origin,
